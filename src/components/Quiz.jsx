...
const Quiz = () => {
    ...
    const [currentQuestion, setCurentQuestion] = React.useState(0)
    ...
    <>
      <header className='flex gap-2 px-2 py-6'>
        {Array.from(Array(questions.length).keys()).map((i) => (
          <div
            key={i}
            className={`h-3 bg-red-600 w-full rounded-md bg-opacity-40 ${
              i <= currentQuestion ? "bg-opacity-100" : ""
            }`}
          ></div>
        ))}
      </header>
          ...
          {showResult ? (
            <div className='result'>
              {(correctCounter / questions.length) * 100 > 50 ? (
                <div className='passed'>
                  ...
                  <h2 className='text-6xl mb-5 font-extrabold text-green-700'>
                    {(correctCounter / questions.length) * 100}%
                  </h2>
                  <p className='mb-5'>
                    You got {correctCounter}/{questions.length}
                  </p>
                  <button className='bg-green-600 px-8 py-2 rounded-full text-white'>
                    Print Certificate
                  </button>
                </div>
              ) : (
                <div className='failed'>
                  ...
                  <h2 className='text-6xl mb-5 font-extrabold text-red-700'>
                    20%
                  </h2>
                  <p className='mb-5'>You go 1 / 5 score</p>
                  <button
            ...
            <div className='question'>
              ...
              <h5 className='font-semibold mb-8'>
                {randomize !== null && randomize[currentQuestion].question_question}
              </h5>
              <ul className='space-y-5 mb-10'>
                {randomize !== null && randomize[currentQuestion].question_choices.map(
                  (item, key) => (
                    <li
                      className={` bg-slate-200 rounded-full ${
                        isActive === key ? "bg-red-700 text-white" : ""
                      } `}
                      key={key}
                    >
                      <button
